...
class User{
    ...
    public function LOGIN_USER(){
        $strSQL = "SELECT user_Email, user_Password FROM user_tb
                   WHERE user_tb.user_Email = :userEmail";
        ...
        $stmt->bindParam(":userEmail", $this->userEmail);
        if ($stmt->execute()) {
            $db_data_email = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($db_data_email['user_Password'] ==($this->userPassword=md5(htmlspecialchars(strip_tags($this->userPassword)))) ) {
                return true;
            }
            return false;
        } else {
